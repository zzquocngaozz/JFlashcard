...
// TODO: lam edit form
export default function AddClassSetDialog({ handleToggle, addClassSet }) {
  const [dueAt, setDueAt] = React.useState({ payload: null, error: null });
  const [searchParam, setSearchParam] = React.useState("");
  const [currentPage, setCurrentPage] = React.useState(1);
  const [listSet, setListSet] = React.useState(null);
  const [pagingList, setPaginList] = React.useState(null);
  const { listExist, loading, mutationing, addSetToClass } =
    useTeacherCreatedSet();
  const handleSearch = (e) => {
    setSearchParam(e.target.value.trim());
  };

  const handleChangeDueAt = (e) => {
    const dueAt = e.target.value;
    setDueAt({
      payload: dueAt,
      error: checkDueAt(dueAt)
        ? null
        : "Hãy để hạn ít nhất 3 ngày để học sinh của bạn học một cách hiệu quả",
    });
  };
  const handleAdd = (flashcardSetId) => {
    if (!checkDueAt(dueAt.payload)) {
      setDueAt({
        ...dueAt,
        error: checkDueAt(dueAt.payload)
          ? null
          : "Hãy để hạn ít nhất 3 ngày để học sinh của bạn học một cách hiệu quả",
      });
      return;
    ...
    addSetToClass(flashcardSetId, dueAt?.payload, addClassSet);
  };
    ...
    <>
      ...
      >
        ...
        <TextField
          ...
          error={Boolean(dueAt.error)}
          helperText={
            !Boolean(dueAt.error)
              ? "Chọn thời hạn để xem được tiến độ học tập của học sinh"
              : dueAt.error
          }
