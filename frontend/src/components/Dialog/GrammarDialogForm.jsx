...
// TODO: lam edit form
export default function GrammarDialogForm({dataInit, handleToggle, onSubmit}) {
    ...
    <>
      <Dialog open={true} sx={{'& .MuiPaper-root':{maxWidth:'calc(100vw - 100px)'}}}>
        <form noValidate onSubmit={handleSubmit(submitForm)} style={{padding:'10px 20px'}}>
          <DialogTitle>{`${!dataInit?"Tạo":"Sửa"} thẻ ngữ pháp`}</DialogTitle>
          <DialogContent sx={{width:'900px', display:'flex', gap:'60px'}}  >
           <Stack flex={6} sx={{width:'100%', height:'100%', flexDirection:'row', justifyContent:"space-between", flexWrap:'wrap', rowGap:"20px", columnGap:"20px"}}>
             ...
             <TextField
               ...
               helperText={(!!errors.hint)?(`Cách chia ${errors?.hint?.message}`):""}
               InputLabelProps={{ shrink: true }}
               variant="standard"
               sx={{width:'100%'}}
             />
             <TextField
               ...
               error={!!errors.note}
               helperText={(!!errors.note)?(`Lưu ý ${errors?.note?.message}`):""}
               InputLabelProps={{ shrink: true }}
               variant="standard"
               sx={{width:'100%'}}
             />
