...
const FolderSingle = ({ folder }) => {
  return (
    <StackCardLink to={"#"}>
      <Stack spacing={2}>
      <StackList ><FolderOpenIcon/><Typography>{folder.title}</Typography></StackList>
      <StackList ><FilterNoneIcon/><Typography>{folder.numberSet + " "} bộ flashcard</Typography></StackList>
      </Stack>
      <StackList >
        <AccessTimeIcon sx={{color:"#079"}}/>
        <Typography>{folder.createdAt}</Typography>
      </StackList>
    </StackCardLink>
  );
};
